...
const Certification = ({ setCertiModal, setSignUpModal }: ModalProps) => {
  const ModalCheck = useRef<HTMLDivElement | null>(null);
  ...
  const dupliEmail = () => {
    axios
      .post(process.env.REACT_APP_BASE_URL + `users/mail/duplicate`, { email: email })
      .then(() => {
        setOKCerti(true);
        ToastSuccess("인증번호가 전송되었습니다.");
        axios.post(process.env.REACT_APP_BASE_URL + `users/mail/signup`, { email: email }).catch((e) => {
  ...
  const CertiEmail = () => {
    axios
      .post(process.env.REACT_APP_BASE_URL + `users/mail/verify`, inputs)
      .then(() => {
      ...
      >
        <_.Container>
          <_.Wrapper>
            ...
            <_.Button
              id="Certi"
              margin={OKCerti ? "120px 0 0 0" : "180px 0 0 0"}
              disabled={!(email && auth_code)}
              onClick={() => {
                CertiEmail();
              }}
            >
